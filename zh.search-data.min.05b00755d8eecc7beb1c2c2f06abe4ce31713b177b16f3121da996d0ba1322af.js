...
「主名」是指文件名中，「点号」 . 之前的部分。这部分内容可以自定，相当于人类的姓名。上图中， 数字逻辑设计第 13 讲 和 Obsidian.1.5.11 等都是文件的主名。 「点号」是指文件名中间最后一个 . 。 「扩展名」是指文件名中，「点号」 . 之后的部分²。扩展名展示着文件的类型，它会告诉操作系统，这个文件应该用什么方式来打开。例如，上图中 本科毕业论文（设计）写作指南（理工类）.doc 中的 doc 就是这个文件的扩展名，它说明这个文件是一个「Word 文档」，应该使用 Word 或 WPS 等软件打开。 如果你的电脑上，文件的扩展名没有被显示（也就是说你只能看到文件的主名），像这样：
请参见下面的操作速查卡进行设置。
