...
如果需要购买 Office，可以前往 微软官网 或者 Microsoft Store 进行购买。如果你是大学生或大学教职工，或许你所在的学校已经为你们批量购买过正版 Office 软件。此时，你可以通过查阅自己学校官网，或者咨询学校网络信息中心 / 正版软件服务中心来了解正版 Office 的获取方式。下图是某学校正版软件平台的截图，在这里可以获取到正版的各版本 Office。
...
`}),e.add({id:3,href:"/missing/new-laptop-setup.html",title:"笔记本新机「开荒」指南",section:"你缺失的那门计算机课",content:` 笔记本新机「开荒」指南 # 本章介绍新笔记本电脑从收货到开箱，从系统首次配置到软件注意事项的完整流程。看完这一部分，你将能找到这些问题的答案：
...
简单验机 # 按提示完成剩余的系统配置步骤并稍作等待，你就能进入新电脑的桌面了。这时我们 千万不要联网（不然前功尽弃，前面花了不少劲来跳过联网步骤呢），而应该在系统内进行一些简单的验机。
...
`}),e.add({id:4,href:"/missing/tools-software.html",title:"工具类软件推荐",section:"你缺失的那门计算机课",content:` 工具类软件推荐 # 本章我们介绍一些好用的「工具类软件」，例如视频播放器、下载工具、PDF 查看器等。我们尽量保证推荐的工具软件体积小巧，界面简洁，功能强大，同时尽量以开源、免费软件代替收费、破解软件。看完这一部分，你将能找到这些问题的答案：
...
`}),e.add({id:5,href:"/missing/basic-maintenance.html",title:"基本维护和安全防护",section:"你缺失的那门计算机课",content:` 基本维护和安全防护 # 上一章我们认识到了「网络世界的险恶」——恶意软件层出不穷、安装捆绑无处不在。在这样的环境之下，掌握电脑的简单维护方法显得尤为重要。阅读完这部分内容，你将找到下面这些问题的答案：
不小心安装了不想要的软件，怎么卸载它们？ 为什么打开有些 app 时，系统总是提示「是否运行此应用对你的电脑进行更改」？ 杀毒软件？安全中心？电脑管家？到底用什么？ 为什么电脑天天都在「更新」？ 网络上不干净的东西这么多，我要怎么才能「洁身自好」？ 掌握一定的电脑维护的方法，例如软件的卸载、流氓软件的排查，以及了解一些电脑的安全维护的策略和措施是保障我们「网上冲浪」安全的重要一环。在这一部分，我们将了解一些常用的电脑维护技巧，以及一些电脑安全方面的常识。
...
对于 安装包 来说，安装包本身的工作就是安装新的软件，而有限的权限禁止了这种行为。 对于一些 专业软件 来说，它需要连接到一些系统级的部件才能工作，而有限的权限禁止了这种行为。 对于一些 「系统优化」类的软件 来说，它本身就是需要更改系统设置的，而有限的权限禁止了这种行为。 此时，程序需要「提升自己的权限」来完成自己的工作，这个过程称为「提权」。而下面展示的这种弹窗（「你要允许此应用对你的设备进行更改吗」，称为「UAC 弹窗」），则是程序在向系统申请「提权」时，系统对用户的提示。这样就能解释，为什么在大多数安装或者卸载软件的时候，系统都会弹出这个窗口询问我们；而只有我们点击【是】，安装或者卸载才能正常继续了。
...
注释 # 如果你在使用中压根没见过这样的窗口（即 UAC 弹窗），那么这一整节的内容对你可能不适用：
`}),e.add({id:6,href:"/missing/manage-storage.html",title:"空间管理大师",section:"你缺失的那门计算机课",content:` 成为空间管理大师 # 本章会为大家在用电脑时非常苦恼的一件事——硬盘空间吃紧，来提供一些建议与小技巧，看完本章，下面的问题你应该有了眉目：
...
程序文件夹 # 你如果留心观察过软件的安装过程，或者仔细阅读了软件的寻找与安装一章，就会知道 Program Files 与 Program Files (x86) 两个文件夹是用来存放软件本体的文件夹。不过，实际上它们存放的是「为所有用户安装的软件」，至于只为一个用户安装的软件，我们后面再说。那么，为什么要分两个文件夹呢？这还要从 CPU 说起。
...
按下 Windows + X，选择【PowerShell】或【终端】； 使用如下语法创建符号链接： ni &lt;链接所在路径&gt; -i SymbolicLink -ta &lt;本体所在路径&gt; 这种方式对文件与文件夹都有效，但 PowerShell 无法识别 %appdata% 这样的特殊路径语法。同样，带空格的路径记得打上双引号。 看上去 New-Item 可以用一个命令搞定文件与文件夹，挺通用，但目前它有一个重大问题——无法处理带西文方括号（「[」和「]」）的路径。所以**如果有路径包含了西文方括号，请使用 mklink。**如果提示什么「权限不足」之类的信息，请用管理员权限再试一次。
...
分区大挪移 # 在笔记本新机「开荒」指南中，我们已经介绍过如何给新电脑从默认分区中拆出一个新分区来。但是，如果你换了块硬盘，或者使用了奇怪的操作，可能就会遇到这样的尴尬事情：
...
练习 # 将一个文件/文件夹移到别处，再在原位置建立符号链接、硬链接及快捷方式，照常使用、访问它，看看三者有什么不同。 尝试用「磁盘清理」工具清一下你的所有分区。 查看你电脑上的分区情况，以及是否存在细小的未分配空间，若有必要，可以试着重新分区。（记得备份数据！） 注释 # 直接在记事本里面点【打开】是不行的，因为「打开」操作调用了文件资源管理器。 `}),e.add({id:7,href:"/missing/recover-from-bsod.html",title:"蓝屏与解决蓝屏",section:"你缺失的那门计算机课",content:` 蓝屏与解决蓝屏 # 这一章，我们将介绍令大多数人都头痛的问题——蓝屏死机，以及解决蓝屏问题的方法。读完本章，你应该可以找到下面这些问题的答案：
...
早期 NT 时代 # 自 1985 年起的几年，微软除了开发自家的 Windows 之外，还在与 IBM 合作开发一款名叫「OS/2」的操作系统，这个系统不是很出名，毕竟它的目标是 IBM 自家的计算机。到了 1988 年 11 月，微软的一个开发团队准备改进当时的 OS/2，使之成为一个兼容 POSIX（可移植操作系统接口）的多用户操作系统，起名「NT OS/2」¹。
...
练习 # 你还见过什么样类似「蓝屏」性质的系统错误界面，它们与「蓝屏」有何异同？ 尝试自己分析一个蓝屏转储文件，如果自己电脑里没有可以找找朋友的电脑（但不必为了这个就把自己的电脑弄蓝屏）。 注释 # 那时 NT 代表「New Technology」，但后来却失去了含义，变成了单纯的代号。 事实上早期 NT 时代中也出现过这些英文，不过它们并没有「蓝屏代码」的地位。而「简洁」时代的蓝屏代码也不全是英文，偶尔会有数字出现。 `}),e.add({id:8,href:"/missing/answers.html",title:"练习题答案",section:"你缺失的那门计算机课",content:` 练习题答案 # 本教程每一章节的末尾都会有数道练习题，其中一些需要你进行实际操作，另一些则需要你动脑思考。这里我们列出那些需要动脑思考的练习题的答案。
...
怎样寻找优质的教程 # 略。
那些好用的键盘快捷键 # 略。
显示器的二三事 # 略。 提示：利用勾股定理，计算出屏幕对角线方向上的像素数，然后除以屏幕尺寸即得 PPI。 略。 用户与微软账号 # 略。 略。 因为 Administrator 的权限太大，打开任何程序都会赋予它很高的权限，而且又没有提示，因此恶意软件可以为所欲为而不被我们知晓；另外，因我们乱改系统设置和文件造成系统损坏的概率也会直线上升。 蓝屏与解决蓝屏 # 提示：Linux 的「Kernel Panic」、Windows 预览版的「绿屏死机」、PlayStation 系列的「红屏错误」等。 略。 成为空间管理大师 # 略； 略； 提示：磁盘管理工具就能查看，但重新分区很可能需要第三方软件。 `}),e.add({id:9,href:"/missing/browsers-and-how-to-choose.html",title:"浏览器——网上冲浪必备",section:"你缺失的那门计算机课",content:` 浏览器——网上冲浪必备 # 浏览器是我们浏览网页的必备工具，它们将互联网上的形形色色的网页呈现在我们面前。不过，各种浏览器之间的门派以及它们之间交错复杂的历史渊源，却并不为多数人所知。
...
练习 # 根据「浏览器界的『血雨腥风』」一节的叙述，复述任意一款主流浏览器（火狐、IE、Chrome）的发展 / 兴衰史。 你正在用什么浏览器？你喜欢它的什么功能？如果你在阅读完这部分内容后想更换一款浏览器并长久使用，你会选择哪一款？ 电脑里安装的浏览器是越多越好吗？为什么？ `}),e.add({id:10,href:"/missing/shortcut-keys.html",title:"那些好用的键盘快捷键",section:"你缺失的那门计算机课",content:` 那些好用的键盘快捷键 # 欢迎来到《你缺计课》进阶篇！在基础篇，我们了解了一些基本的电脑使用的常识；在软件篇，我们收获了一大批优秀的软件并了解了它们的故事。而在进阶篇，我们会逐步迈入一个「深水区」——这一部分的一些内容可能不那么容易理解。不过，我们会尽量用容易看懂的语言去介绍它们。 相信大家对「快捷键」早已不陌生。这一节我们要介绍的快捷键，并不是像 Ctrl + C 「复制」，Ctrl + V 「粘贴」这样的我们早已耳熟能详的常用快捷键，而是那些很多新手未必一定知道但十分好用的快捷键和键盘用法。 在开始之前，如果你不知道类似「Ctrl + Shift + S」的快捷键怎么按，请参阅 一些约定与预备知识 。
...
练习 # 逐个尝试上面的快捷键，然后将那些你用得顺手的快捷键应用在你的工作、学习和生活中。 `}),e.add({id:11,href:"/missing/computer-and-its-components.html",title:"认识你的电脑",section:"你缺失的那门计算机课",content:` 认识你的电脑 # 在这一部分，我们将对「电脑」这种神奇的黑箱做一个简要的、整体的介绍。看完这一部分，你将可以找到这些问题的答案：
...
英特尔公司现在主要的 CPU 产品线称作「酷睿」（Core），而「酷睿」系列又分成了 4 个档次——「i3」「i5」「i7」和「i9」。如果你现在在用笔记本电脑，不妨瞄一眼键盘下方是否有这样一个蓝色（也有可能是灰色）的贴纸： AMD 公司现在主要的 CPU 产品线称作「锐龙」（Ryzen），而「锐龙」系列也分成了 3 个档次——「R5」「R7」和「R9」²。如果你现在用笔记本电脑，不妨瞄一眼键盘下方是否有这样一个红色的贴纸： 我们常常说一台电脑是「双核」「四核」的，这里的「双核」「四核」也是处理器中的概念。这些处理器在一片芯片放了多个「核心」，相当于一个个协同起来的独立的小处理器。例如，「双核」意味着在一枚处理器芯片上集成了两个核心，相当于两个大脑协同工作，当我们需要用电脑同时做很多事情的时候就有所裨益。同理，「四核」「八核」就是在一个芯片上集成了四个甚至是八个核心。
...
内存 / RAM # 紧接着，我们介绍能直接与处理器交流的部件——内存，英文简写「RAM」。上一小节提到，处理器相当于大脑，但与大脑不同的是，处理器只能 处理 数据，而这些待处理的数据，需要依赖外部的元件来临时存储。内存就是用来临时存储数据的。
...
集成显卡，英特尔称「核芯显卡」。显卡被安排在处理器的同一片芯片上，性能相对较差，能应付大多数工作，但游戏、制图等特定工作就不太行了。集成显卡功耗很低，而且省去了用户单独购买显卡的麻烦。 独立显卡，简称「独显」。显卡芯片仍然是一片独立的芯片，有自己的供电和外围元件。这样的显卡性能比较强，但换来的是更高的功耗、更大的体积（游戏本为什么重）和更多的发热等。 今天，全世界生产 独立显卡 的厂商主要有两家，一家叫「英伟达」（Nvidia），它生产的显卡俗称「N 卡」；另一家是前文提到过造处理器的「AMD」，它生产的显卡俗称「A 卡」。如果你有涉足过硬件交流圈，玩家所说的「RTX 3060」「GTX 1080 Ti」等都是英伟达显卡的型号，「RX 6800 XT」「RX 580」等都是 AMD 显卡的型号。上图是目前英伟达的顶级游戏显卡 RTX 4090。
...
在 Windows 10 / 11 中，打开【任务管理器】，切换到【性能】选项卡，可以看到一个更详细的硬件运行状态，其中【硬盘】相关栏目展示了你设备的硬盘数量和类型（机械硬盘 HDD、固态硬盘 SSD）。尝试查看你电脑的硬盘容量和类型。
你可以通过按 Ctrl + Shift + Esc 来打开「任务管理器」。也可以右击【开始按钮】，选择【任务管理器】来打开它。
你对「电路」的认知有多少？你是否好奇 CPU 是怎么运作的？从开关、导线、电池、灯泡组成的最简单「电路」到几乎无所不能「电脑」之间到底发生了什么奇妙的变化？但限于《你缺计课》篇幅，我们是没法告诉你这些的。但是，你若有兴趣，可以去学习「电路电子技术」「数字电路与数字逻辑」「计算机体系结构」等相关内容。近年来，国际形势风云变幻，我国在芯片领域仍然存在许多短板。我们希望越来越多的有志青年能投身于包括但不限于体系结构、硬件组成、数字电路乃至微电子、半导体材料等领域，为我国芯片行业「补上短板」贡献自己的力量。
注释 # 事实上，芯片的「设计」和「制造」两件事是不一样的，就像能设计出房屋的建筑师不一定会到工地上去砌墙。英特尔能够自行完成从设计到制造整条流水线，而 AMD 只能完成设计，它的处理器是由专门负责制造芯片的厂商（例如台积电）生产的。 其实还有 R3，但是很少在消费市场见到。 一开始，集成显卡并不是集成到处理器中的，而是集成在主板上的一个芯片之中（称为「北桥」），后来才集成到了处理器里面。 关于 Windows 的一点点更新历史，可以参见蓝屏与解决蓝屏一章。 `}),e.add({id:12,href:"/missing/software-installation.html",title:"软件的寻找与安装",section:"你缺失的那门计算机课",content:` 软件的寻找与安装 # 软件的下载安装一直以来是困扰许多「电脑小白」的大问题——从哪里下？怎么下？下完怎么装？装完怎么办？什么，软件要收费？破解是什么？……这一章将对国内互联网环境下 Windows 软件的下载、安装和配置做一个简单的介绍。看完这一部分，你或许可以找到下面这些问题的答案，并慢慢对「怎么寻找软件」有一个初步的了解。
...
如上图，我们搜索「WPS 下载」，结果中：
第一条是「重庆天极网络有限公司」提供的广告。 第二条是「腾讯电脑管家」提供的广告，我们进去之后八成会下载到「腾讯电脑管家」而不是「WPS」。 第三条是「马鞍山超聪网络科技」提供的广告。 第四条是「北京奥蓝德信息科技」提供的广告。 上面这四条 全部是 广告。这意味着进入这些页面，很可能下载不到干净的「WPS」，而只能下载到一堆垃圾。
...
看网址。一般官网的网址都是企业或者软件的名字。例如： WPS 的官网是 www.wps.cn。 QQ 的官网是 im.qq.com。微信的则是 weixin.qq.com。（腾讯首页： qq.com） 网易云音乐的官网是 music.163.com。（网易官网： 163.com） …… 排除法。没有哪个软件厂商的名字是叫做「xx 软件站」「xx 下载站」「xx 软件园」的。带有这些名字的 全部 是第三方下载站。 语义判断。一般广告网站的标题都与搜索关键词没有任何实质上的联系。我们不妨再看上面的搜索结果中的前四条广告： 第一条：「办公软件下载客户端 2021 年正版免费下载」，只字不提「WPS」或者「金山办公」。（WPS 是金山公司开发的办公软件） 第二条：「wsp 办公软件下载-2021 官方版-电脑管家软件中心」，挂羊头卖狗肉 而且挂错了。 第三条、第四条类似第一条。 进入软件的官方网站之后，我们搜索「全部产品」「软件下载」之类的选项，就可以找到我们所需要的软件的安装包。例如从 WPS 的官网上下载 WPS 软件：
...
一个光秃秃的 exe 文件或者 msi 文件。我们直接双击这个文件就能启动安装进程。 一个压缩包，例如 zip 或 rar 文件。我们需要解压缩这个压缩包到某处，然后在解压出来的文件中找到名字类似「setup.exe」或「install.exe」的程序双击打开。 一个扩展名是 iso 的文件。我们右击这个 iso 文件，选择【打开方式】→【文件资源管理器】，然后在弹出的新窗口中找到名字类似「setup.exe」或「install.exe」的程序双击打开¹。 启动安装器后，我们一般按提示【下一步】操作即可完成安装。
...
演示：「高速下载器」到底会干什么 # 请不要在自己电脑上尝试打开这种「高速下载器」
...
注释 # 当你这样打开 iso 文件后，这个文件会临时被「映射」成电脑里的一个新「分区」，打开【此电脑】就能发现它。在你安装完成后，可以打开【此电脑】，右键这个分区选择【弹出】来让这个分区消失。 `}),e.add({id:13,href:"/missing/file-and-file-management.html",title:"文件与文件管理",section:"你缺失的那门计算机课",content:` 文件与文件管理 # 在这一部分，我们将重新介绍你可能所不知道的「文件」以及「文件」的管理，以及一些实用的文件管理的技巧。看完这一部分，你将可以找到下面这些问题的答案：
...
一个是「盘符」，盘符是一个英文字母加上冒号 : 构成的。我们所称呼的「C 盘」「D 盘」正是指盘符中的那个字母。如上图，左方分区的盘符是 C: ，右方分区的盘符是 D: 。盘符一般在被指定之后就不方便更换了。在今天，Windows 系统中的盘符都是从字母 C 开始¹的。 另一个是「卷标」，这是一个可选的标识符，它是一定长度的文本。在上图中，C 盘的卷标是「Windows」，D 盘的卷标是「Files」。盘符的作用是让系统和软件识别分区，而卷标的作用是帮助我们用户更加直观地了解分区的作用，因而卷标是可以随时更换的。事实上，通过右键某一分区，选择【重命名】，就可以更换卷标了。如果你什么都不写，它默认叫做「本地磁盘」。 我们在 认识你的电脑 中提到，操作系统本身也是一个大软件，那么这个软件放在硬盘上的哪呢？对于 Windows 而言，整个 Windows 系统默认放在 C 盘里。双击打开电脑 C 盘，你会看到一些你可能不熟悉的文件夹，例如 Windows 文件夹，Windows 系统自己的许多文件就存在其中。
...
在本章的后续部分，以及 软件的寻找与安装 中，我们会详细介绍，怎样把我们的软件以及其他东西放在 C 盘以外的地方，以及这样做的其他好处。
文件、文件名和文件类型 # 「文件」是数据存储在硬盘上的形式。这么说可能有些抽象，直观地说，文件就是我们每天都打交道的东西：
文件有自己的「名字」，称为「文件名」。在 Windows 系统中，文件名可以分成三个部分：
「主名」是指文件名中，点号 . 之前的部分。这部分内容可以自定，相当于人类的姓名。上图中， 如何让富婆爱上我 和 一夜暴富指南 等都是文件的主名。 「点号」是指文件名中间的那个 . 。 「扩展名」是指文件名中，点号 . 之后的部分²。扩展名展示着文件的类型，它会告诉操作系统，这个文件应该用什么方式来打开。例如，上图中 每天一个长寿秘诀.txt 中的 txt 就是这个文件的扩展名，它说明这个文件是一个「文本文档」，应该使用「记事本」打开。 扩展名也是可以人为改变的，但这样往往会出问题——试想，对于上图中的 高等数学不挂速成.mp4，它本来是一个 mp4 文件，即「视频文件」，应该用看视频的软件打开。如果你强行把它改成 txt，系统就会用「记事本」来打开一个「视频文件」——用错误的工具打开文件。
...
文件夹、路径和目录 # 「文件夹」是一个用来存放其他文件的结构。不妨想象一下现实中的「文件夹」：
...
D:\\missing\\源文件\\第三章.docx 这一长串东西表示出 第三章.docx 这个文件在电脑中的具体位置，我们把一长串东西称为 第三章.docx 这个文件的「路径」（严格来说叫做「绝对路径」）。不难发现，路径是从「分区」（也就是「盘」）开始，用反斜杠 \\ 作为分隔，一级一级文件夹地展开，最后到具体的文件。类似的，不只是文件，文件夹的路径也可以用这样的方式来表示，例如
...
D:\\Program Files (x86)\\Netease\\CloudMusic\\cloudmusic.exe 因此，你双击打开这个「网易云音乐」快捷方式，就会打开 cloudmusic.exe 这个文件。如果你删掉了这个快捷方式，它并不会影响 cloudmusic.exe 这个文件，不会影响你电脑上安装的网易云音乐本身。
...
右击压缩文件，选择【解压⁵到当前文件夹】或者【解压到 &lt;文件名&gt;\\】。这两个选项的不同是： 【解压到当前文件夹】会把压缩文件里的内容直接放在压缩文件的同一目录下。例如，如果一个压缩文件 archive.zip 里面有 a.txt 和 b.txt 两个文件，选择此选项，解压后 a.txt 和 b.txt 都和 archive.zip 在同一级目录。
...
扩展名 用什么软件打开 软件主程序的路径在哪 txt 记事本 C:\\Windows\\system32\\notepad.exe docx Word C:\\Program Files\\Microsoft Office\\root\\Office16\\WINWORD.EXE mp3 网易云音乐 D:\\Program Files (x86)\\Netease\\CloudMusic\\cloudmusic.exe …… …… …… 有了这张表，系统就能自动地帮我们选择文件对应的打开方式。有时，我们不想要用这张表帮我们预置的方式来打开文件。比如，打开 jpg 图片的默认方式是「图片」软件，但如果我们想 暂时 用「Photoshop」来打开它，我们可以这样做：
...
系统安装更新，即 Windows 更新之后，这张表可能会被重置而丢失一些项目，造成某些文件突然无法打开。这有可能是由于你使用的对应软件并不是原版，而是某种「精简版」。 新安装一个软件时，软件安装程序可能写入新的表项，来为即将安装的新软件的文件类型配置打开方式。 Windows 的各种恶性 bug 和恶心操作。例如，Windows 会时不时用 Microsoft Edge 代替其他软件来打开网页文件和 pdf 文件等。 （微软你坏事做尽） 如果你的文件关联某天突然失效，即某种类型的文件突然无法打开，但对应的软件工作正常，那么可以考虑是否是上面的原因。
...
更改用户文件夹的存储位置 * # 上文提到了一个叫做 文档 的文件夹，它是系统预置的一个用户文件夹。在介绍这一部分之前，我们先简单提一下「用户文件夹」是什么。
...
我们将这里文本框中
C:\\Users\\&lt;你的用户名&gt;\\ 这一部分**（注意：最后一个 Desktop、 Documents 之类的名字不要动！）**⁶改成
...
注释 # 这是因为 A 和 B 两个盘符在过去是留给「软盘」的，但软盘早就已经成为历史了，不过这个习惯却保留了下来。 有时候我们称呼文件的扩展名时也会把点号包含进去。下面两种说法是等价的： 某文件的扩展名是 txt。 某文件的扩展名是 .txt。 千万不要手动把一个正常文件的扩展名改成 lnk，否则就很难改回来了。 具体来说，大多数压缩工具都可以解压 rar 格式的压缩包，但只有 WinRAR 这一款压缩工具可以制作这种格式的压缩包；而这款工具是收费的。详见 压缩文件与压缩工具。 部分压缩工具称「解压」为「提取」，是同一个词（Extract）的不同翻译。 如果你不小心把一个用户文件夹的位置设置成了某个磁盘的根目录（比如说 D:\\），会触发 Windows 系统的一个 bug，导致很难再改回来。（辣鸡 Windows） `}),e.add({id:14,href:"/missing/screens-and-their-secrets.html",title:"显示器的二三事",section:"你缺失的那门计算机课",content:` 显示器的二三事 # 这一章我们介绍一个 认识你的电脑 不曾介绍过的电脑硬件——显示器。作为电脑与我们直接打交道的「第一线」，显示器的显示效果，直接影响着我们操作电脑的体验。看完这一部分，你或许能找到下面这些问题的答案：
...
分辨率和尺寸：想要更加「细腻」的显示效果，在尺寸相同的条件下选分辨率高的。比如，13 寸的笔记本屏幕，分辨率最好在 1920 × 1080 以上；21 寸的台式机显示器，分辨率最好在 2560 × 1440 以上。 色域与色准：若对色彩有要求，选择「DCI-P3 90%」「100% sRGB」「99% sRGB」「72% NTSC」等高色域的显示器；对色准有要求，选择厂商宣称「出厂校色」并且 Delta E 比较小（例如，Delta E &lt; 2）的显示器。 色深：日常使用则选择 8 位显示器，但若有图像处理等专业需求，选择色深 10 位及以上的显示器。 面板类型：**选择 IPS 屏幕或者 VA 屏幕，预算充足、追求色彩可以考虑 OLED 屏幕。**TN 屏非常「瞎眼」，除非你知道自己在做什么，否则不要选择。 刷新率：若你追求游戏性能，在配置（尤其是显卡）足够的情况下，选择 144 Hz、120 Hz 等高刷新率的；反之，60 Hz、75 Hz 和 90 Hz 的显示器已经足够满足你的需求。 练习 # 查看你电脑显示器的分辨率。可以通过在桌面上右键 →【显示设置】→【显示分辨率】来查看。 了解你电脑显示器的尺寸。尺寸指的是屏幕对角线的长度，一般单位是「英寸」（inch）。然后，结合上一题查到的分辨率，计算你屏幕的 PPI。 查找你电脑显示器的色深。可以去桌面上右键 →【显示设置】→【高级显示器设置】找找。 在电商平台上查找显示器 / 笔记本电脑，看看厂商有没有宣传屏幕的色域、色准、色深和刷新率信息。 `}),e.add({id:15,href:"/missing/preamble.html",title:"序",section:"你缺失的那门计算机课",content:` 序 # 按理来说，对于所谓「Z 世代」的年轻人，熟练地使用电脑应该是他们的生活必备技能。
...
这份教程会在文中简称自己为「《你缺计课》」或者「本教程」。本教程的得名参考了麻省理工大学（MIT）的《计算机教育中缺失的一课 (The Missing Semester of Your CS Education)》。
`}),e.add({id:16,href:"/missing/archive-formats-and-tools.html",title:"压缩文件与压缩工具",section:"你缺失的那门计算机课",content:` 压缩文件与压缩工具 # 在 文件与文件管理 一章中，我们简要介绍了「压缩文件」：利用各种各样的压缩工具，我们可以将一组零散的文件和文件夹「打包」成一个体积稍小的文件，这个体积稍小的文件就是「压缩文件」。压缩文件有许多不同的种类，用来完成压缩流程的压缩工具也各有特色。看完这一部分，你或许可以找到这些问题的答案：
...
常见的压缩文件格式 # 在开始介绍这些格式之前，我们先引入一个叫做「压缩率」的概念。假设一堆文件原来的总体积是 100 MB，在压缩成某种格式的压缩文件后，体积变成了 86 MB，就称这时的压缩率是 86%。压缩率不仅与压缩文件格式有关，也与源文件本身的性质有关，但我们仍然可以以「大多数工作情况下的压缩率水平」的高低来笼统地比较几种不同的压缩文件格式。
...
练习 # 简要用自己的话复述「压缩」这个过程。 自己制作几个压缩文件，然后分别用「解压到当前文件夹」「解压到 xxxxxx\\」来解压它们，体会这两种解压方式的区别。思考 Bandizip 的「智能解压」到底有多「智能」。 你正在使用什么压缩软件？它的使用体验（界面美观度、操作难易度、是否有广告……）如何？ 注释 # 加密的压缩文件可以选择「加密文件名」，这样的压缩包如果没有密码，我们将完全看不到压缩包内有什么东西；反之即使没有密码，我们也能看到压缩包内文件的列表。 `}),e.add({id:17,href:"/missing/first-things-first.html",title:"一些约定与预备知识",section:"你缺失的那门计算机课",content:` 一些约定与预备知识 # 尽管《你缺计课》自认为受众是「电脑小白」，但我们依然不得不要求读者有一些最最基本的知识和操作经验。此外，为了便于后续的讲述，我们会进行一些约定。
...
先按住 Windows 键（Windows 键上印有 Windows 徽标，一般来说这个键在 Ctrl 和 Alt 之间）不要松手； 再按住 Shift 键，同样不要松手； 接着按一下 S 键，然后松开全部按键。 F1 — F12 功能键的使用说明 # 在很久以前，人们为了增加电脑键盘的使用效率，在键盘的最顶端设计了一排按键，它们就是 F1 — F12 功能键：
...
有关「重启」的说明 # 由于自 Windows 8 以来的 Windows 系统引入了「快速启动」的机制，现在「重启」这个过程并不等价于「先关机再开机」的过程。
...
「设置」和「控制面板」 # 在 Windows 10 和 Windows 11 中，有两个不同的地方可以对系统设置进行更改：
...
练习 # 计算 1 GB 等于多少 KB？等于多少字节？假设一个汉字占两个字节，1 GB 大约可以记录多少个汉字？ 尝试计算，一个按「1000 进位」计算得到容量为 64 GB 的 U 盘，它按「1024 进位」得到的容量是多少？ 在自己电脑上尝试这些快捷键： Windows + Shift + S （仅限 Windows 10 / 11） Ctrl + Shift + Esc Windows + D 了解并试验自己笔记本电脑的 F1 — F12 功能键的拓展功能。 `}),e.add({id:18,href:"/missing/user-and-ms-account.html",title:"用户与微软账号",section:"你缺失的那门计算机课",content:` 用户与微软账号 # 本章我们介绍 Windows 系统中的「用户」概念，以及微软账号的相关事项。看完这一部分，你或许可以找到这些问题的答案：
为什么我自己的电脑还有「用户」？ 什么是微软账号（Microsoft 账户¹）？我为什么被要求注册 / 登录微软账号？ 在 基本维护和安全防护 中提到的「UAC 弹窗」和「以管理员身份运行」究竟是什么？ 登录微软账号有什么好处？ 「用户」的概念是现代计算机操作系统的一个重要组成部分。尽管在今天，绝大多数电脑都仅为个人使用，「多用户」的理念似乎正渐渐模糊，但我们依然有必要了解这套机制的运作过程。
...
Windows 操作系统中的「用户」 # 在我们第一次启动 Windows 系统（参见 笔记本新机「开荒」指南 ）时，系统会要求我们新建一个用户，并设置它的用户名和密码，它将作为你自己所使用的用户²。打开 C:\\用户\\ 目录，你可以看到一个以它命名的文件夹，以及一个 公用 文件夹：
...
练习 # 查看自己的用户名，检查自己是以本地帐户登录的还是以微软帐号登录的。 检查自己的设备是否有启用设备加密。一般来说，只有部分品牌系列的「商务本」才会默认启用这个功能。 为什么说将 Administrator 作为主用户登录无异于悬着一把「达摩克利斯之剑」？ 注释 # 注意，由于微软的疏忽，Windows 中多用「帐户」一词，然而正确的写法应是「账户」，但为了忠于原文，我们未作更改。 如果你在第一次开机时直接登录了微软帐号，那么系统会先用你的微软帐号的前 5 位作为用户名新建一个本地帐户~~，然后你就会发现一个巨难看的本地用户名~~，然后迅速将它连接到你的微软帐号（参见后文）。整个过程对你来说完全无感。 本地存储的用户名是不会因为绑定到微软帐号而改变的。即，绑定前后，打开 C:\\用户\\ 看到的文件夹名字是一样的。 `}),e.add({id:19,href:"/missing/how-to-find-solutions.html",title:"遇到问题怎么办",section:"你缺失的那门计算机课",content:` 遇到问题怎么办 # 在日常使用电脑的过程中，我们会遇到各种各样的问题。这里的「问题」，指的是电脑并没有按照我们的想法工作，有时还伴随着意料之外的提示语句。学会借助互联网等工具解决问题，是帮助我们更好地使用电脑的重要一环。看完这一部分，你将能找到下面这些问题的答案：
...
`}),e.add({id:20,href:"/missing/how-to-find-tutorials.html",title:"怎样寻找优质的教程",section:"你缺失的那门计算机课",content:` 怎样寻找优质的教程 # 在软件篇开始的时候我们就强调了，《你缺失的那门计算机课》从来不会是任何一款具体软件的教程。这是由这份教程的编写初衷决定的——每个人所需要掌握的软件都不一样，要学习的具体方面更是千差万别，本教程无法，也不可能成为一个人人都能取用的大杂烩。相反地，我们在这里向大家介绍「怎样寻找优质的教程」。 一份好的软件教程能帮助我们更快、更熟练地使用软件。今天，互联网上的内容多如牛毛，但像《你缺计课》这样用心编写的优质教程却少之又少。寻找优质软件教程，如同「沙里淘金」。这一部分将向大家介绍一些可能更容易找到优质教程的方法。
