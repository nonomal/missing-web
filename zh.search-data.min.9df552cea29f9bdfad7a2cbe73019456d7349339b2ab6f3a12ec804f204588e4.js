...
`}),e.add({id:4,href:"/missing/bring-intelligence-to-machines.html",title:"从加减乘除到 AI 对话",section:"你缺失的那门计算机课",content:` 从加减乘除到 AI 对话 # 欢迎来到《你缺失的那门计算机课》超越篇！在这一部分，我们将带领大家在了解「电脑最好怎么用」的基础上，探索无限的可能性——今天，人们利用电脑这一强大的工具，创造了一个璀璨斑斓的世界。人工智能、云计算、网络安全、大数据……这些词汇，或许你早已在各种各样的地方听说过，但你是否真正了解它们？我们将用通俗易懂的语言，带领大家走进这些新颖的领域，探索它们的奥秘。 这一章我们将介绍超越篇的第一个领域——人工智能，也就是 AI（Artificial Intelligence）。AI 技术让电脑不再是冷冰冰的机器，而拥有了一定模仿人类思维的能力。今天，诸如 AI 对话、AI 绘画、AI 写作等技术正在极大地提高人们的生产效率。阅读完这一章，你或许能找到下面这些问题的答案：
我想体验一下智能的 AI 对话，去哪儿呢？ 从加减乘除这样的基本运算到 AI 技术，中间到底发生了什么？ `}),e.add({id:5,href:"/missing/tools-software.html",title:"工具类软件推荐",section:"你缺失的那门计算机课",content:` 工具类软件推荐 # 本章我们介绍一些好用的「工具类软件」，例如视频播放器、下载工具、PDF 查看器等。我们尽量保证推荐的工具软件体积小巧，界面简洁，功能强大，同时尽量以开源、免费软件代替收费、破解软件。看完这一部分，你将能找到这些问题的答案：
...
`}),e.add({id:6,href:"/missing/basic-maintenance.html",title:"基本维护和安全防护",section:"你缺失的那门计算机课",content:` 基本维护和安全防护 # 上一章我们认识到了「网络世界的险恶」——恶意软件层出不穷、安装捆绑无处不在。在这样的环境之下，掌握电脑的简单维护方法显得尤为重要。阅读完这部分内容，你将找到下面这些问题的答案：
...
`}),e.add({id:7,href:"/missing/manage-storage.html",title:"空间管理大师",section:"你缺失的那门计算机课",content:` 成为空间管理大师 # 本章会为大家在用电脑时非常苦恼的一件事——硬盘空间吃紧，来提供一些建议与小技巧，看完本章，下面的问题你应该有了眉目：
...
练习 # 你还见过什么样类似「蓝屏」性质的系统错误界面，它们与「蓝屏」有何异同？ 尝试自己分析一个蓝屏转储文件，如果自己电脑里没有可以找找朋友的电脑（但不必为了这个就把自己的电脑弄蓝屏）。 注释 # 那时 NT 代表「New Technology」，但后来却失去了含义，变成了单纯的代号。 事实上早期 NT 时代中也出现过这些英文，不过它们并没有「蓝屏代码」的地位。而「简洁」时代的蓝屏代码也不全是英文，偶尔会有数字出现。 `}),e.add({id:9,href:"/missing/answers.html",title:"练习题答案",section:"你缺失的那门计算机课",content:` 练习题答案 # 本教程每一章节的末尾都会有数道练习题，其中一些需要你进行实际操作，另一些则需要你动脑思考。这里我们列出那些需要动脑思考的练习题的答案。
...
怎样寻找优质的教程 # 略。
那些好用的键盘快捷键 # 略。
显示器的二三事 # 略。 提示：利用勾股定理，计算出屏幕对角线方向上的像素数，然后除以屏幕尺寸即得 PPI。 略。 用户与微软账号 # 略。 略。 因为 Administrator 的权限太大，打开任何程序都会赋予它很高的权限，而且又没有提示，因此恶意软件可以为所欲为而不被我们知晓；另外，因我们乱改系统设置和文件造成系统损坏的概率也会直线上升。 蓝屏与解决蓝屏 # 提示：Linux 的「Kernel Panic」、Windows 预览版的「绿屏死机」、PlayStation 系列的「红屏错误」等。 略。 成为空间管理大师 # 略； 略； 提示：磁盘管理工具就能查看，但重新分区很可能需要第三方软件。 `}),e.add({id:10,href:"/missing/browsers-and-how-to-choose.html",title:"浏览器——网上冲浪必备",section:"你缺失的那门计算机课",content:` 浏览器——网上冲浪必备 # 浏览器是我们浏览网页的必备工具，它们将互联网上的形形色色的网页呈现在我们面前。不过，各种浏览器之间的门派以及它们之间交错复杂的历史渊源，却并不为多数人所知。
...
练习 # 逐个尝试上面的快捷键，然后将那些你用得顺手的快捷键应用在你的工作、学习和生活中。 `}),e.add({id:12,href:"/missing/computer-and-its-components.html",title:"认识你的电脑",section:"你缺失的那门计算机课",content:` 认识你的电脑 # 在这一部分，我们将对「电脑」这种神奇的黑箱做一个简要的、整体的介绍。看完这一部分，你将可以找到这些问题的答案：
...
注释 # 事实上，芯片的「设计」和「制造」两件事是不一样的，就像能设计出房屋的建筑师不一定会到工地上去砌墙。英特尔能够自行完成从设计到制造整条流水线，而 AMD 只能完成设计，它的处理器是由专门负责制造芯片的厂商（例如台积电）生产的。 其实还有 R3，但是很少在消费市场见到。 一开始，集成显卡并不是集成到处理器中的，而是集成在主板上的一个芯片之中（称为「北桥」），后来才集成到了处理器里面。 关于 Windows 的一点点更新历史，可以参见蓝屏与解决蓝屏一章。 `}),e.add({id:13,href:"/missing/software-installation.html",title:"软件的寻找与安装",section:"你缺失的那门计算机课",content:` 软件的寻找与安装 # 软件的下载安装一直以来是困扰许多「电脑小白」的大问题——从哪里下？怎么下？下完怎么装？装完怎么办？什么，软件要收费？破解是什么？……这一章将对国内互联网环境下 Windows 软件的下载、安装和配置做一个简单的介绍。看完这一部分，你或许可以找到下面这些问题的答案，并慢慢对「怎么寻找软件」有一个初步的了解。
...
注释 # 当你这样打开 iso 文件后，这个文件会临时被「映射」成电脑里的一个新「分区」，打开【此电脑】就能发现它。在你安装完成后，可以打开【此电脑】，右键这个分区选择【弹出】来让这个分区消失。 `}),e.add({id:14,href:"/missing/file-and-file-management.html",title:"文件与文件管理",section:"你缺失的那门计算机课",content:` 文件与文件管理 # 在这一部分，我们将重新介绍你可能所不知道的「文件」以及「文件」的管理，以及一些实用的文件管理的技巧。看完这一部分，你将可以找到下面这些问题的答案：
...
`}),e.add({id:17,href:"/missing/archive-formats-and-tools.html",title:"压缩文件与压缩工具",section:"你缺失的那门计算机课",content:` 压缩文件与压缩工具 # 在 文件与文件管理 一章中，我们简要介绍了「压缩文件」：利用各种各样的压缩工具，我们可以将一组零散的文件和文件夹「打包」成一个体积稍小的文件，这个体积稍小的文件就是「压缩文件」。压缩文件有许多不同的种类，用来完成压缩流程的压缩工具也各有特色。看完这一部分，你或许可以找到这些问题的答案：
...
练习 # 简要用自己的话复述「压缩」这个过程。 自己制作几个压缩文件，然后分别用「解压到当前文件夹」「解压到 xxxxxx\\」来解压它们，体会这两种解压方式的区别。思考 Bandizip 的「智能解压」到底有多「智能」。 你正在使用什么压缩软件？它的使用体验（界面美观度、操作难易度、是否有广告……）如何？ 注释 # 加密的压缩文件可以选择「加密文件名」，这样的压缩包如果没有密码，我们将完全看不到压缩包内有什么东西；反之即使没有密码，我们也能看到压缩包内文件的列表。 `}),e.add({id:18,href:"/missing/first-things-first.html",title:"一些约定与预备知识",section:"你缺失的那门计算机课",content:` 一些约定与预备知识 # 尽管《你缺计课》自认为受众是「电脑小白」，但我们依然不得不要求读者有一些最最基本的知识和操作经验。此外，为了便于后续的讲述，我们会进行一些约定。
...
练习 # 计算 1 GB 等于多少 KB？等于多少字节？假设一个汉字占两个字节，1 GB 大约可以记录多少个汉字？ 尝试计算，一个按「1000 进位」计算得到容量为 64 GB 的 U 盘，它按「1024 进位」得到的容量是多少？ 在自己电脑上尝试这些快捷键： Windows + Shift + S （仅限 Windows 10 / 11） Ctrl + Shift + Esc Windows + D 了解并试验自己笔记本电脑的 F1 — F12 功能键的拓展功能。 `}),e.add({id:19,href:"/missing/user-and-ms-account.html",title:"用户与微软账号",section:"你缺失的那门计算机课",content:` 用户与微软账号 # 本章我们介绍 Windows 系统中的「用户」概念，以及微软账号的相关事项。看完这一部分，你或许可以找到这些问题的答案：
...
`}),e.add({id:21,href:"/missing/how-to-find-tutorials.html",title:"怎样寻找优质的教程",section:"你缺失的那门计算机课",content:` 怎样寻找优质的教程 # 在软件篇开始的时候我们就强调了，《你缺失的那门计算机课》从来不会是任何一款具体软件的教程。这是由这份教程的编写初衷决定的——每个人所需要掌握的软件都不一样，要学习的具体方面更是千差万别，本教程无法，也不可能成为一个人人都能取用的大杂烩。相反地，我们在这里向大家介绍「怎样寻找优质的教程」。 一份好的软件教程能帮助我们更快、更熟练地使用软件。今天，互联网上的内容多如牛毛，但像《你缺计课》这样用心编写的优质教程却少之又少。寻找优质软件教程，如同「沙里淘金」。这一部分将向大家介绍一些可能更容易找到优质教程的方法。
